...
class ProductsRepository implements IProductsRepository {
  ...
  public async updateQuantity(
    ...
    const findProducts = await this.ormRepository.findByIds(idProducts);

    const updatedProducts: Product[] = Object.assign(findProducts);

    products.map((product: IUpdateProductsQuantityDTO) => {
      const productUpdated = updatedProducts.find(
        updatedProduct => updatedProduct.id === product.id,
      );

      if (productUpdated) productUpdated.quantity -= product.quantity;
    });
